...
st.set_page_config(page_title="NarrAItive", page_icon="🤖", layout="wide", )
...
def voice_format_for_selectbox(voice):
    """
    Format the voice for the selectbox widget
    :param voice:
    :return:
    """
    return f"{voice[1]} {[voice_i for voice_i in voice[2].values()]}"
...
def main():
    ...
    load_story_button = st.sidebar.button('Load Story')
    delete_button = st.sidebar.button('Delete Story')
    # generate new picturebook button
    new_picturebook_button = st.sidebar.button('Generate new Picturebook', on_click=set_stage, args=(1,))
    if load_story_button:
    ...
    if (st.session_state.stage >= 1) and (st.session_state.stage <= 4):
        if not openai_api_key.startswith('sk-'):
            ...
            voice_category = st.selectbox("Premade or cloned voice?", get_all_voice_categories())
            voices = get_all_voices_for_category(voice_category)
            selected_voice = st.selectbox("Which voice?", voices, format_func=voice_format_for_selectbox)
            ...
            st.button("Generate Story", on_click=set_stage, args=(3,))
    ...
    if st.session_state.stage > 4:
        ...
        if st.button('Previous', on_click=set_stage, args=(5,)):
            st.session_state.index = (st.session_state.index - 1) % number_segments

        if st.button('Next', on_click=set_stage, args=(5,)):
            st.session_state.index = (st.session_state.index + 1) % number_segments
        ...
        with Image.open(img_path) as img:
            # print image in streamlit and rescale to 70 % of original size
